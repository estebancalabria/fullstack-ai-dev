...
def input_int(mensaje: str, mensaje_error: str = "Entrada invalida. Debe ingresar un numero entero.") -> int:
    while True:
...
def agregar_alumno(db: list):
    ...
    cantidad_cursos = input_int("Ingrese la cantidad de cursos que ha tomado el alumno: ")
...
def eliminar_alumno(db: list):
    id_alumno = input_int("Ingrese el ID del alumno que desea eliminar: ")
    for alumno in db:
...
def modificar_alumno(db: list):
    id_alumno = input_int("Ingrese el ID del alumno que desea modificar: ")
    for alumno in db:
        if alumno["id"] == id_alumno:
